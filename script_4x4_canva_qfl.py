...
OUTPUT_PDF        = 'yearbook_4x4_canva_qfl.pdf'
OUTPUT_PDF_CLEAN  = 'yearbook_4x4_canva_qfl_ready.pdf'
...
# ── Ghostscript Canva Fix ────────────────────────────────────────────────────
def fix_for_canva(input_pdf: str, output_pdf: str):
    """
    Re-processes the PDF through Ghostscript, which re-embeds all fonts with
    complete glyph width tables. This fixes character spacing and overlapping
    issues in Canva.
    """
    gs_bin = shutil.which('gs') or shutil.which('gswin64c') or shutil.which('gswin32c')
    if not gs_bin:
        print("\n[ERROR] Ghostscript not found. Canva fix aborted.")
        return False

    print(f"\n[INFO] Running Ghostscript Vectorization (Outlining Fonts)...")
    # -dNoOutputFonts: This is the 'Nuclear Option' that turns all text into vector shapes.
    # It ensures Canva cannot clump the text because it is no longer text—it's a drawing.
    cmd = [
        gs_bin,
        '-dBATCH',
        '-dNOPAUSE',
    ...
    try:
        ...
        print(f"[DONE] Canva-ready PDF saved to: {output_pdf}")
        return True
...
# LARGE QUOTE STYLE Version (QFL)
QUOTE_STYLE = ParagraphStyle('Quote', fontSize=8, leading=10, alignment=TA_CENTER, textColor=colors.HexColor('#555555'), fontName=FONT_REGULAR)
...
def format_unicode_for_reportlab(text: str) -> str:
    ...
    if not result_chunks: result_chunks = [text]
    ...
    for chunk in result_chunks:
        ...
        for char in chunk:
            ...
            font_needed = None
            if 0x3040 <= code <= 0x309F or 0x30A0 <= code <= 0x30FF or 0x4E00 <= code <= 0x9FFF: font_needed = "NotoSansJP"
            elif 0x0900 <= code <= 0x097F: font_needed = "NotoSansDevanagari"
...
def load_default_image():
    ...
    try: return crop_and_resize(Image.open(DEFAULT_IMAGE), IMAGE_RATIO_W, IMAGE_RATIO_H)
    except Exception: return None

def build_cell(name, student_id, quote, photo_url, default_pil_img, col_width):
    rl_img = None
    is_placeholder = (not photo_url or pd.isna(photo_url) or "null" in str(photo_url).lower() or str(photo_url).strip() == "" or "drive.google.com" not in str(photo_url))
    if not is_placeholder:
        pil_img = load_image(photo_url)
        if pil_img: rl_img = image_to_rl(crop_and_resize(pil_img, IMAGE_RATIO_W, IMAGE_RATIO_H), col_width)
    if not rl_img and default_pil_img: rl_img = image_to_rl(default_pil_img, col_width)
...
def build_page_table(page_students, page_start, total, col_width, usable_w, usable_h, default_pil_img):
    ...
    # EVEN SPACING FIX: Explicitly set row heights to divide space equally
    row_height = usable_h / ROWS_PER_PAGE
    ...
    tbl = Table(table_data, colWidths=[col_width] * COLS_PER_PAGE, rowHeights=[row_height] * len(table_data))
    # PHOTO ALIGNMENT FIX: Use TOP alignment and explicit TOPPADDING for vertical headroom
...
def main():
    ...
    if args.update:
        ...
        if student_idx is None:
            print(f"[ERROR] Student ID {target_id} not found."); return

        target_page_idx = student_idx // STUDENTS_PER_PAGE
        page_students = students[target_page_idx*STUDENTS_PER_PAGE : (target_page_idx+1)*STUDENTS_PER_PAGE]
        tmp_pdf = "tmp_single_page.pdf"
        c_tmp = rl_canvas.Canvas(tmp_pdf, pagesize=A4)
        render_page(c_tmp, page_students, target_page_idx*STUDENTS_PER_PAGE, total, page_w, page_h, margin, col_width, usable_w, usable_h, default_pil_img)
        c_tmp.save()

        if os.path.exists(OUTPUT_PDF):
            reader, writer = PdfReader(OUTPUT_PDF), PdfWriter()
            new_page = PdfReader(tmp_pdf).pages[0]
            for i in range(len(reader.pages)):
                writer.add_page(new_page if i == target_page_idx else reader.pages[i])
            with open(OUTPUT_PDF, "wb") as f: writer.write(f)
            os.remove(tmp_pdf)
            print(f"[DONE] Page {target_page_idx + 1} updated.")
        else: os.rename(tmp_pdf, OUTPUT_PDF)
    else:
        c = rl_canvas.Canvas(OUTPUT_PDF, pagesize=A4)
        for i in range(0, total, STUDENTS_PER_PAGE):
            print(f"[INFO] Page {(i//STUDENTS_PER_PAGE)+1}...")
            render_page(c, students[i:i+STUDENTS_PER_PAGE], i, total, page_w, page_h, margin, col_width, usable_w, usable_h, default_pil_img)
        c.save()
        print(f"[DONE] Saved to {OUTPUT_PDF}")

    # Automatically run the Canva fix
    fix_for_canva(OUTPUT_PDF, OUTPUT_PDF_CLEAN)
